...
export default function RaceCard({ round, circuitId, country, date }: IRaceCard) {
  ...
  const clean = {
    ...
    circuitId: circuitId
      ...
      .map((e) => capitalize(e))
      .join(" "),
    country: (countriesCorrections[country] || country).toLowerCase().replace(/\s/g, "-")
  };
  ...
  return (
    ...
    >
      <div className="z-20 drop-shadow-card-text-md">
        <h1 className="text-[22px] font-medium">{countriesCorrections[country] || country}</h1>
